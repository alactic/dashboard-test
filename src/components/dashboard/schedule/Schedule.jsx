...
const Schedule = () => {
  ...
  const onNavEmoji = (point) => {
    if (point === "left") {
      const startIndex =
        emojiIndex.start === 0 ? Emoji.length - 1 : emojiIndex.start - 1;
      const endIndex =
        emojiIndex.end === 0 ? Emoji.length - 1 : emojiIndex.end - 1;
      ...
      const endIndex =
        Emoji.length === emojiIndex.end + 1 ? 0 : emojiIndex.end + 1;
      const startIndex =
        Emoji.length === emojiIndex.start + 1 ? 0 : emojiIndex.start + 1;
      ...
      <div className="schedule-content">
        <form onSubmit={onSubmit}>
          <div className="space-between">
            ...
            <span className="dash-calender cursor">
              <Dashboard /> <Calender />
            </span>
          </div>
          ...
          <div>
            <input placeholder="Create new" className="schedule-input" />
          </div>

          <div className="emoji">
            <span className="cursor" onClick={() => onNavEmoji("left")}>
              <ArrowLeft />
            </span>
            <div className="emoji-items">
              {emojiValue.map((val, i) => (
                <span key={i} className="emoji-item">
                  {String.fromCodePoint("0x" + val)}
                </span>
              ))}
            </div>
            <span className="cursor" onClick={() => onNavEmoji("right")}>
              <ArrowRight />
            </span>
          </div>
          <div>
            ...
            <div className="colloborator-items">
              <span className="colloborator-item">
                <img
                  ...
                  alt="Colloborator1"
                />
                <span className="collo-name name1">Angela</span>
                <span className="cursor">
                  <CancelIcon color="#5051F9" />
                </span>
              </span>
              <span className="colloborator-item">
                <img
                  ...
                  alt="Colloborator2"
                />
                <span className="collo-name name2">Chris</span>
                <span className="cursor">
                  <CancelIcon color="#1EA7FF" />
                </span>
              </span>
              <span className="add-collob cursor">+</span>
          ...
          <div>
            ...
            <div className="form-input start-date">
              <span className="calender cursor">
                <Calender />
              </span>
          ...
          <div className="save-btn">
            <input type="submit" value="Save" className="btn cursor" />
          </div>
        </form>
        <div>
